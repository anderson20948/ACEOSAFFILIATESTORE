...
app.get('/cdn/bootstrap.css', (req, res) => {
  if (CDN_CONFIG.enabled) {
    ...
    res.sendFile(path.join(nodeModulesDir, 'bootstrap/dist/css/bootstrap.min.css'));
  }
});
...
app.get('/cdn/bootstrap.js', (req, res) => {
  if (CDN_CONFIG.enabled) {
    ...
    res.sendFile(path.join(nodeModulesDir, 'bootstrap/dist/js/bootstrap.bundle.min.js'));
  }
});
...
app.get('/cdn/chart.js', (req, res) => {
  if (CDN_CONFIG.enabled) {
    ...
    res.sendFile(path.join(nodeModulesDir, 'chart.js/dist/chart.umd.js'));
  }
});
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    // 3. Automated support for localhost and 127.0.0.1 dev origins
...
function sendFrontendFile(res, filePath) {
  res.sendFile(filePath, (err) => {
    if (err) {
      logger.error("Failed to send frontend file", {
        filePath,
        error: err.message,
        stack: err.stack
      });
...
function isPassportAuthenticated(req) {
...
// Capture process-level failures so the real root cause is always logged.
...
process.on('uncaughtException', (err) => {
  ...
  // Optionally exit here if you want the process manager to restart the service.
  // process.exit(1);
});
